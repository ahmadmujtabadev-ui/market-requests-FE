...
//         deadline,
//         platforms: platforms.includes('Other') ? [...platforms.filter(p => p !== 'Other'), otherPlatform] : platforms,
...
//               <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-manrope transition-all ${
//                 currentStep >= 1 
//                   ? 'bg-black text-white' 
...
//           {/* Step 1: Template Selection & Project Title */}
//           {currentStep === 1 && (
//             <>
//               {/* Template Selection */}
...
export default function SubmitRequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Prepare platforms array with "Other" value if needed
      const finalPlatforms = platforms.includes('Other') 
        ? [...platforms.filter(p => p !== 'Other'), otherPlatform.trim()]
        : platforms;

      // Prepare file URLs (in real app, these would be uploaded to storage first)
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {currentStep === 2 && (
            <div className="bg-white rounded-lg border border-[#EEEEEE] p-6">
              ...
              <p className="text-sm text-[#595959] font-roboto mb-6" style={{ fontWeight: 400 }}>
                Specify when you need this and where you'll use it
              </p>
